...
- `tests/animation-set.test.mjs` (included in `test:creature-pipeline`): all 26 canine clips, coverage of every primary donor clip, unchanged mesh/rest data, loop closure, per-clip glTF validation and bounded asset growth. Historical GLBs are regression fixtures, excluded from Pages.
