...
rep('new THREE.BoxGeometry(CELL-.2,.06,CELL-.2)','new THREE.BoxGeometry(2.2,.06,2.2)');
...
code=code.replaceAll('lvlCol[c.lvl+1]','lvlCol[clamp(Math.floor(c.lvl)+1,0,3)]').replaceAll('lc[c.lvl+1]','lc[clamp(Math.floor(c.lvl)+1,0,3)]');
...
const integration=fs.readFileSync('game/integration.js','utf8')+'\n'+fs.readFileSync('game/walls.js','utf8')+'\n'+fs.readFileSync('game/spatial.js','utf8')+'\n'+fs.readFileSync('game/combat.js','utf8')+'\n'+fs.readFileSync('game/lifecycle.js','utf8')+'\n'+fs.readFileSync('game/rework.js','utf8')+'\n'+fs.readFileSync('game/portals.js','utf8')+'\n'+fs.readFileSync('game/navigation.js','utf8')+'\n'+fs.readFileSync('game/controls.js','utf8')+'\n'+fs.readFileSync('game/settlement.js','utf8')+'\n'+fs.readFileSync('game/frontier-structures.js','utf8')+'\n'+fs.readFileSync('game/frontier-npcs.js','utf8')+'\n'+fs.readFileSync('game/frontier-war.js','utf8')+'\n'+fs.readFileSync('game/frontier-ui.js','utf8')+'\n'+fs.readFileSync('game/expedition-geometry.js','utf8')+'\n'+fs.readFileSync('game/expedition-ai.js','utf8')+'\n'+fs.readFileSync('game/voxel-models.js','utf8')+'\n'+fs.readFileSync('game/expedition-world.js','utf8')+'\n'+fs.readFileSync('game/rolling-world.js','utf8')+'\n'+fs.readFileSync('game/rounded-plateaus.js','utf8')+'\n'+fs.readFileSync('game/traversal-performance.js','utf8')+'\n'+fs.readFileSync('game/frame-pacing.js','utf8')+'\n'+fs.readFileSync('game/creature-forge.js','utf8');
code=code.replaceAll('G.cells[x][z].lvl*LSTEP','heightAt(p.x,p.z)');
code=code.replace('for(const [w,h,x,z] of [[S+2', 'for(const [w,h,x,z] of (RUN_SETTINGS.dynamic?[]:[[S+2').replace('[1,S+2,S/2,0]])','[1,S+2,S/2,0]]))');
code=code.replaceAll('const p=cellToWorld(x,z);const y=',"const p=cellToWorld(x,z);const rng=mulberry32(hashStr(G.seed+'/tree/'+p.x+'/'+p.z));const y=").replaceAll('const p=cellToWorld(x,z);const s=.7+rng()',"const p=cellToWorld(x,z);const rng=mulberry32(hashStr(G.seed+'/rock/'+p.x+'/'+p.z));const s=.7+rng()");
...
rep('rock.setMatrixAt(i,dummy.matrix)',"rock.setMatrixAt(i,dummy.matrix);G.generatedBodies.push({...bodyCircle(p.x,p.z,heightAt(p.x,p.z),.85*s*1.2,1.7*s,'rock'),draw:[{mesh:rock,index:i}]})");
...
html=html.replace(/(<b>Controls<\/b>[\s\S]*?id="helpToggle">hide<\/span>)[\s\S]*?(?=\n  <\/div>)/, '$1<div>W/S move · Space jump · Shift sprint · V view</div><div>Third person: LMB drag free look · RMB drag steer</div><div>Hold both mouse buttons to move forward</div><div>A/D turn · hold RMB + A/D to strafe · wheel zoom</div><div>First person: mouse look · hold Alt for cursor</div><div>Top view: RMB rotates; WASD moves</div><div>1–6/click: select move; repeat or LMB: cast</div><div>RMB a move: toggle automatic use</div><div>Tab: target · Escape: clear target · E: interact</div><div>B: build/combat · 1–9 defenses · 0 portal</div><div>LMB places · E sells · Enter starts a wave</div><div>Menu/Pause pauses; switching tabs does not</div>');
...
html=html.replaceAll('release Enter to drop','release Space to drop');
