...
rep('dummy.position.set(p.x+jx,y+.5,p.z+jz);',"G.generatedBodies.push(bodyCircle(p.x+jx,p.z+jz,y,.24,3.3*s,'tree'));dummy.position.set(p.x+jx,y+.5,p.z+jz);");
rep('rock.setMatrixAt(i,dummy.matrix)',"rock.setMatrixAt(i,dummy.matrix);G.generatedBodies.push(bodyCircle(p.x,p.z,G.cells[x][z].lvl*LSTEP,.85*s*1.2,1.7*s,'rock'))");
...
code=code.replaceAll('B founds a bastion.','B selects a core; aim and click to place.').replaceAll('0x120f18','0x87999f');
