...
Use slow playback to check paw placement against the grid, stance-foot sliding, shoulder/hip deformation and the transition across the loop boundary. This is inspection tooling, not a foot-locking solver or animation editor. Arbitrary external clips cannot yet be imported through the forge.

Voltfang now includes **Quaternius wolf walk — retargeted trial** alongside the original Tripo walk. Select **walk**, then **Walk clip (preview)** to compare them. Switching clips resets animation time while preserving the pause/speed controls. The choice is local to that preview actor; gameplay retains the manifest's original walk, and reopening the preview restores that default. Neither clip selection nor playback consumes API credits.
...
New compatible canine generation jobs also append **Quaternius wolf walk — canine profile v3 trial** offline before installation. This uses the same worker locally and on GitHub Actions, adds no provider calls, and keeps the provider walk as the gameplay default. Voltfang retains all three clips for comparison: original, initial experiment, and shared-profile trial. The shared profile is experimental; it has been exercised on one real generated rig plus synthetic changes to scale, heading and limb proportions, not a second independently generated creature.

`rig-profiles.mjs` separates semantic anatomy roles from provider bone names. Revision 3 supports the current Tripo canine hierarchy and the bundled Quaternius wolf donor. It resolves all four paw controls, checks required skinned joints and parent chains, measures body heading and leg lengths, and scales root motion to the target. Paw rotations use each donor foot's lowest sampled pose as its stance reference. Meshes, textures, weights, bind poses and existing clips are preserved. Unsupported/ambiguous rigs keep the original asset and record a skip reason; unexpected processing failures or invalid output stop installation. Humanoids do not receive this canine trial.

Revision 2 adds a procedural secondary-motion layer to the donor walk: a small head turn (±5°) and nod (±2°), plus lateral tail sway growing from ±9° at the base to ±18° at the tip with delayed follow-through. These are shared profile settings, not Voltfang-specific offsets or animation generated by Quaternius. The measured body frame determines the axes; unmapped tails receive no invented joints. The motion is baked into the exported walk, loops continuously, and leaves leg tracks unchanged. It is an art-directed loop, not reactive gaze, tail physics or idle animation. Use `{ "secondaryMotion": false }` in offline calibration JSON to compare the donor transfer alone; the report records the settings and mapped tail-joint count.
...
Revision 3 centers the neutral tail before applying secondary motion. It measures the horizontal base-to-tip direction against the body's rear axis and preserves the tail's curl/elevation. Short, nearly vertical, or strongly folded tails are flagged for review instead of guessing a yaw. Optional `tailYawDegrees` overrides the inference (±60°; zero disables centering). The report retains the resolved angle and whether it was inferred or explicit.

Voltfang's v3 preview uses a visually checked `headYawDegrees: -10` and an inferred tail correction of approximately +25.46°. The head offset is this asset's calibration data, not a default for future creatures. These corrections are baked into the v3 walk only; rest/idle and the original provider clip still retain the imported pose. The default gameplay walk remains the provider clip while animation review continues.
