...
export type SaveGame={version:1;savedAt:string;seed:string;classId:string;config:{seed:string;size:number;hilliness:number;trees:number;scale:number;dynamic?:boolean;maxSize?:number};state:{wave:number;gold:number;phase:string;[key:string]:unknown};[key:string]:unknown};
export function parseSave(raw:string):SaveGame{
 ...
 const c=s.config;if(!c||!Number.isInteger(c.size)||c.size<64||c.size>1024||c.size%2||!Number.isFinite(c.scale)||c.scale<.25||c.scale>4||!Number.isFinite(c.hilliness)||c.hilliness<0||c.hilliness>1||!Number.isFinite(c.trees)||c.trees<0||c.trees>.6)throw new Error('Invalid saved world settings.');
