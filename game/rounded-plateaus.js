...
function plateauVariation(x,z){
 ...
 const seed=(hashStr(G.seed)>>>0)%1000;
 return weight*(Math.sin(x*.11+seed)*Math.cos(z*.095-seed)*.11+Math.sin(x*.047+z*.071+seed)*.07);
}
function plateauSurface(cx,cz,x,z){
 const c=cellAt(cx,cz);if(!c)return 0;
 const center=cellToWorld(cx,cz),half=CELL/2,rim=CELL*.075,raw=plateauRaw(cx,cz,x,z);let flatWeight=1,toe=0;
 for(const [dx,dz]of [[1,0],[-1,0],[0,1],[0,-1]]){
  ...
  const round=Math.pow(1-clamp(d/rim,0,1),2);
...
heightAt=function(x,z){
 ...
 const [cx,cz]=worldToCell(V3(x,0,z));let h=plateauSurface(cx,cz,x,z);
...
buildTerrainMesh=function(){
 ...
 const vertices=[],uv=[],colors=[],top=[],walls=[];
 ...
 for(let cx=Math.max(0,px-radius);cx<=Math.min(WORLD-1,px+radius);cx++)for(let cz=Math.max(0,pz-radius);cz<=Math.min(WORLD-1,pz+radius);cz++){
  const center=cellToWorld(cx,cz),start=vertices.length/3;
  ...
  for(const [dx,dz]of [[1,0],[-1,0],[0,1],[0,-1]]){
   ...
   for(let i=0;i<n-1;i++){
    ...
    const rock=new THREE.Color(0x929282),a=vertex(ax,ah,az,rock,true),b=vertex(bx,bh,bz,rock,true),c=vertex(ax,Math.min(ah,al),az,rock,true),d=vertex(bx,Math.min(bh,bl),bz,rock,true);walls.push(a,c,b,b,c,d);
   }
  }
 }
