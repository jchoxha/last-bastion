...
function* gridPlanSteps(from,to,r=.45){const start=worldToCell(from),end=worldToCell(to);if(!inGrid(...start)||!inGrid(...end))return null;const keyOf=(x,z)=>z*WORLD+x,first=keyOf(...start),last=keyOf(...end),cost=new Map([[first,0]]),parent=new Map(),closed=new Set(),queue=new NavHeap();queue.push(first,0);let found=false;
 while(queue.length){const [id]=queue.pop();if(closed.has(id))continue;if(id===last){found=true;break;}closed.add(id);const x=id%WORLD,z=Math.floor(id/WORLD);for(const [dx,dz,w]of NBRS8){yield;const nx=x+dx,nz=z+dz,j=keyOf(nx,nz);if(!inGrid(nx,nz)||closed.has(j)||!tileEdge(x,z,nx,nz,r))continue;const weight=1+((hashStr(G.seed+'/road/'+(nx-HALF)+'/'+(nz-HALF))>>>0)%1000)/1666,score=cost.get(id)+w*weight;if(score<(cost.get(j)??Infinity)){cost.set(j,score);parent.set(j,id);queue.push(j,score+Math.hypot(nx-end[0],nz-end[1]));}}}
 ...
 const path=[],anchor=tileAnchor(...start,r);if(!anchor)return null;if(from.distanceTo(anchor)>.15){if(walkSegment(from,anchor,r,'nav'))path.push(anchor.clone());else{const local=yield* localRouteSteps(from,anchor,r);if(!local.length||!walkSegment(local.at(-1),anchor,r,'nav'))return null;path.push(...local,anchor.clone());}}
...
function gridPlan(from,to,r=.45){const job=gridPlanSteps(from,to,r);let step;do{step=job.next();}while(!step.done);return step.value;}
