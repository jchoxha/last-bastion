...
function* foliageJob(){
 if(!rollingEnabled())return;const group=new THREE.Group(),grass=[],bushes=[],flowers=[],p=G.player.pos.clone(),spacing=2.8,centerX=Math.floor(p.x/spacing),centerZ=Math.floor(p.z/spacing),seed=hashStr(G.seed),rand=(x,z,k)=>{let h=Math.imul(x^seed,374761393)^Math.imul(z,668265263)^Math.imul(k,1274126177);h=Math.imul(h^(h>>>13),1274126177);return((h^(h>>>16))>>>0)/4294967295;};
 for(let x=centerX-34;x<=centerX+34;x++)for(let z=centerZ-34;z<=centerZ+34;z++){if(z===centerZ-34)yield;const wx=(x+rand(x,z,1))*spacing,wz=(z+rand(x,z,2))*spacing;if(Math.hypot(wx-p.x,wz-p.z)>94||!inGrid(...worldToCell(V3(wx,0,wz))))continue;if(G.sites.some(s=>Math.hypot(wx-s.corePos.x,wz-s.corePos.z)<CELL*3.3)||G.towers.some(t=>Math.hypot(wx-t.pos.x,wz-t.pos.z)<buildRadius(t.b)+1)||(colony().landmarks||[]).some(l=>Math.hypot(wx-l.pos.x,wz-l.pos.z)<11))continue;const biome=biomeAt(wx,wz),chance=rand(x,z,3);if(biome==='desert'&&chance>.13||biome==='frost'&&chance>.3)continue;const entry=[wx,heightAt(wx,wz),wz,rand(x,z,4),biome];grass.push(entry);if(chance<.055){bushes.push(entry);bushes.push([wx+.34,entry[1]-.08,wz+.2,entry[3]*.6,biome],[wx-.25,entry[1]-.08,wz-.25,entry[3]*.65,biome]);}else if(chance>.96)flowers.push(entry);}
 ...
 function* batch(points,geo,color,type){const mat=new THREE.MeshStandardMaterial({color,roughness:1,side:THREE.DoubleSide}),mesh=new THREE.InstancedMesh(geo,mat,points.length);for(let i=0;i<points.length;i++){if(i%128===0)yield;const [x,y,z,r,biome]=points[i];dummy.position.set(x,y+(type==='bush'?.35:type==='flower'?.35:0),z);dummy.rotation.set(0,r*Math.PI*2,0);dummy.scale.setScalar(type==='bush'?.55+r*.65:type==='flower'?.07:.55+r);if(type==='bush')dummy.scale.y*=.7;dummy.updateMatrix();mesh.setMatrixAt(i,dummy.matrix);mesh.setColorAt(i,new THREE.Color(biome==='frost'?0xa8b8a0:type==='flower'?0xd7c47b:biome==='desert'?0xa69657:0x688849).multiplyScalar(.8+r*.35));}mesh.receiveShadow=true;mesh.castShadow=type==='bush';group.add(mesh);}
 yield* batch(grass,tuft,0xffffff,'grass');yield* batch(bushes,new THREE.IcosahedronGeometry(1,1),0xffffff,'bush');yield* batch(flowers,new THREE.IcosahedronGeometry(1,0),0xffffff,'flower');removeFoliage();G.world.add(group);G.foliage=group;G.foliageCenter=p.clone();
}
function seedFoliage(){const job=foliageJob();while(!job.next().done){}}
const rollingMotion=updatePlayer;
updatePlayer=function(dt){rollingMotion(dt);if(!rollingEnabled())return;if(plateauMode()){streamPlateauTerrain();streamFoliage();return;}if(!G.groundCenter||G.groundCenter.distanceTo(G.player.pos)>36){if(G.terrain){G.world.remove(G.terrain);G.terrain.geometry.dispose();(Array.isArray(G.terrain.material)?G.terrain.material:[G.terrain.material]).forEach(m=>m.dispose());}buildTerrainMesh();}if(!G.foliageCenter||G.foliageCenter.distanceTo(G.player.pos)>20)seedFoliage();};
