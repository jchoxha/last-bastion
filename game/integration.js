...
function expandTerrain(){const max=RUN_SETTINGS.terrainMode==='rolling'?Infinity:Math.min(1024,RUN_SETTINGS.maxSize||1024);if(WORLD>=max)return;const old=WORLD,next=Math.min(max,WORLD+16),offset=(next-old)/2;if(offset<1)return;
